...
def kfold_cv(train,y,kfolds,model):
    # train=trainset.copy()
    val_result=np.zeros(len(train))
    for nfolds,(train_index,val_index) in enumerate(kfolds.split(train.values)):
        # print(train_index,val_index)
        # print('\n')
        trn_data = train.iloc[train_index][:]
        val_data = train.iloc[val_index][:]
        
        trn_target = y.iloc[train_index].values.ravel()
        val_target = y.iloc[val_index].values.ravel()
        
        model.fit(trn_data, trn_target)
        val_result[val_index] = model.predict(val_data).ravel()
        return val_result
...
if __name__ == "__main__":
    ...
    # transform_pipe = Pipeline([('gen_cl', general_cleaner()),
    #                     ('proc', processing_forest),
    #                     ('scaler', dfp.df_scaler(method='robust')),
    #                     ('dropper', drop_columns(forest=True))])

    model=RandomForestRegressor(n_estimators=1500, max_depth=30, 
                                max_features='sqrt',
    ...
    # tmp=train_set.copy()
    # trn=transform_pipe.fit_transform(tmp,y)
    # model.fit(trn,y)
    # tmp = test_set.copy()
